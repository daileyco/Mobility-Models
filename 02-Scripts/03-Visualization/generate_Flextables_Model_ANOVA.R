...
## load data
load("./03-Output/01-Tables/table_model_anovas_totalcommuters.rds")
load("./03-Output/01-Tables/table_model_anovas_internalcommuters.rds")
load("./03-Output/01-Tables/table_model_anovas_gravity.rds")
...
ma.list <- mget(ls(pattern="^fit[.]anovas[.].+$")) %>%
...
caps <- paste0("Model Analyses of Variance, ",
               c("Gravity", "Internal Commuters", "Total Commuters"),
               " Models")



## flextable formatting
ft.list <- lapply(1:length(ma.list), 
                   function(fti){
                     ...
                     ft[which(duplicated(ft[,1:2])),2] <- NA
...
for(i in 1:length(ft.list)){
  ...
  print(temp, 
        target = paste0("./03-Output/01-Tables/table_model_anovas_", 
                        names(ft.list)[i], 
                        ".docx"))
...
## save
save(ft.list, 
     file = "./03-Output/01-Tables/flextables_model_anova.rds")
