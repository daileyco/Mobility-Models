...
table.gravity.fits <- table.gravity.fits %>%
  ...
  mutate(Model = "Gravity, All Population Categories Pairing") %>%
...
table.gravity.fits.simple <- table.gravity.fits.simple %>%
  mutate(Period = factor(Period, levels = c("2011-2020", "2011-2015", "2016-2020"), ordered = TRUE)) %>%
  mutate(Model = "Gravity, Large Population Category Pairings") %>%
  select(Period, Model, everything()) %>%
  arrange(Period, Model) %>%
  mutate(across(!Period, ~as.character(.x)))
...
table.uo.fits <- table.uo.fits %>%
  ...
  mutate(Model = factor(Model, levels = c("UO", "OO", "OPS", "Radiation"), ordered = TRUE)) %>%
  arrange(Period, `Surrounding Population, s_{ij}`,`Total Commuters, T_{i}`, Model) %>%
  mutate(across(!Period, ~as.character(.x)))



table.uo.fits.basic <- table.uo.fits %>%
...
t.list <- c(list(gravity_popcats = table.gravity.fits, 
                 gravity_ll = table.gravity.fits.simple, 
                 gravity_full = bind_rows(table.gravity.fits, 
                                          table.gravity.fits.simple)%>%
                   arrange(Period)), 
            list(uo_full = table.uo.fits, 
                 uo_basic = table.uo.fits.basic), 
            list(all=bind_rows(table.gravity.fits, 
                               table.gravity.fits.simple,
                               table.uo.fits.basic)%>%
                   arrange(Period)))
...
caps <- paste0("Measures of Fit for ", 
               c("Gravity Models including Interaction with All Population Size Categories Pairings", 
                 "Gravity Models including Interaction with Large Population Size Category Pairings", 
                 "Gravity Models",
                 "Universal Opportunity Models by Choice of Sij and Ti",
                 "Universal Opportunity Models", 
                 "Both Gravity and Universal Opportunity Models"))
...
ft.list <- lapply(1:length(t.list), 
                   function(fti){
                     ...
                     alluscol <- which(names(ft)=="All US")
                     ...
                     ft %>%
                       ...
                       #### text left
                       align(part = "body", j=1:{alluscol-1}, align = "left") %>%
                       #### numbers right
                       align(part = "body", j=alluscol:ncol(ft), align = "right") %>% 
...
for(i in 1:length(ft.list)){
  ...
  print(temp, 
        target = paste0("./03-Output/01-Tables/table_model_fit_", 
                        names(ft.list)[i], 
                        ".docx"))
...
## save
save(ft.list, 
     file = "./03-Output/01-Tables/flextables_model_fit.rds")
