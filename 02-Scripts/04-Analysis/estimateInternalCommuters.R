# script to estimate total internal commuters (workers) based on population size, time period, and census region
...
## subset for only internal commutes
od <- od %>% 
...
### remove time period and time period interaction
fit6 <- update(fit5, ~.-period-log.POPESTIMATE.origin:period)
summary(fit6)
...
fit.anovas <- lapply(mget(ls(pattern="fit[0-9]{1}")), 
  ...
  mutate(Model = row_number(), 
         Terms = c("~log(Origin Population)", 
                   ...
                   "- Time Period - (Time Period)*log(Origin Population)")) %>%
...
### rename
fit.anovas.internalcommuters <- fit.anovas
...
fit.ests[nrow(fit.ests)+1,] <- t(c("Time Period", "2011-2015 (REF)", rep(NA, 6)))
fit.ests[nrow(fit.ests)+1,] <- t(c("(Time Period)*log(Origin Population)", "2011-2015 (REF)", rep(NA, 6)))
fit.ests[nrow(fit.ests)+1,] <- t(c("Origin Census Region", "Midwest (REF)", rep(NA, 6)))
fit.ests[nrow(fit.ests)+1,] <- t(c("(Origin Census Region)*log(Origin Population)", "Midwest (REF)", rep(NA, 6)))

fit.ests <- fit.ests %>%
  arrange(Parameter, Level)

### rename
fit.ests.internalcommuters <- fit.ests



## generate predictions for best fit model
od$log.Internal.Commuters_pred <- predict(fit5, 
                                          od, 
                                          type = "response")

preds.internalcommuters <- od %>%
  select(c(1:5,43))
