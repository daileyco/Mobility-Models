...
The following script processes a simple table with census region designations for each state:

```{r}
source("./02-Scripts/01-Data-Wrangling/process_Data_Spatial_Census_Regions.R", echo = T)
```



Finally, this script combines those coordinates with the population coordinates from the census data and formats all of the as an `sf` spatial data object:
...
These models are all fit to the full data spanning 2011-2020 and the entire US, and each use the same distance threshold which was calibrated with the full data (i.e., 2011-2020 for the entire US) using the gravity model with only distance threshold interaction optimized with the objective function on the raw data scale. 
...
The following script fits a simpler gravity model (base+distance_threshold+large_populations) to each subset of the data using each data subset's tuned distance threshold:

```{r}
source("./02-Scripts/04-Analysis/fitGravity_simple.R", echo = T)
```
...
### Tuning Parameters
This script visualizes the optimized parameter values for each combination of time period and region across the different gravity models and objective function scales:
```{r}
source("./02-Scripts/03-Visualization/generate_Figures_Gravity_Distance_Thresholds.R", echo = T)
```

and in tables:
```{r}
source("./02-Scripts/03-Visualization/generate_Table_Gravity_Tuning.R", echo = T)
```


This script visualizes the optimized parameter values for each combination of time period and region across choices of $S_{ij}$ and $T_i$ and objective function scales:
```{r}
source("./02-Scripts/03-Visualization/generate_Figures_UO_Parameters.R", echo = T)
```

and in tables:

```{r}
source("./02-Scripts/03-Visualization/generate_Table_UO_Tuning.R", echo = T)
```


Tuning tables for both gravity and UO models are formatted in this script:

```{r}
source("./02-Scripts/03-Visualization/generate_Flextables_Model_Tuning.R", echo = T)
```
...
```{r}
source("./02-Scripts/03-Visualization/generate_Flextables_Model_ANOVA.R", echo = T)
```
...
The estimated power parameters for the simpler model are shown visually in a series of figures which are created with the following script:


```{r}
source("./02-Scripts/03-Visualization/generate_Figures_Gravity_Coefs_simple.R", echo = T)
```
...
Along with coefficient estimates for the models of total commuters and internal commuters, these power parameter estimates are shown in tables using these scripts:

```{r}
source("./02-Scripts/03-Visualization/generate_Tables_Gravity_Fit.R", echo = T)
source("./02-Scripts/03-Visualization/generate_Flextables_Model_Coefs.R", echo = T)
```
...
The gravity models fit stats are compiled alongside the coefficient estimates done just before. 

The UO models fit stats are tabulated here:
```{r}
source("./02-Scripts/03-Visualization/generate_Table_UO_Fit.R", echo = T)
```


and all fit stats tables are formatted here:

```{r}
source("./02-Scripts/03-Visualization/generate_Flextables_Model_Fits.R", echo = T)
```


The model fits are compared visually in figures with this script:

```{r}
source("./02-Scripts/03-Visualization/generate_Figures_Model_Fits.R", echo = T)
```
...
I aggregate and clean predictions of commuter flow from each model with each "setting" in this script:
```{r}
source("./02-Scripts/01-Data-Wrangling/process_Data_Predictions_Flow.R", echo = T)
```


And finally, figures showing predicted flows against observed flow quantiles across models and data subsets are created in this script:

```{r}
source("./02-Scripts/03-Visualization/generate_Figures_Flows_ObsVPred.R", echo = T)
```
